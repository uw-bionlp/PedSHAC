...
def get_args():
    ...
    parser.add_argument("--per_device_test_batch_size", type=int, default=8, help="the per device test batch size")
    parser.add_argument("--gradient_accumulation_steps", type=int, default=2, help="the gradient accumulation steps")
...
def evaluate_peft_model(sample):
    # generate summary
    outputs = model.generate(input_ids=sample["input_ids"].unsqueeze(0).cuda(), do_sample=False, top_p=0.9, max_new_tokens=max_target_length)
...
if __name__ == "__main__":
    ...
    if args.mode=='train':
        ...
        output_file=f"predictions/{args.experiment_name}_predictions.json"
